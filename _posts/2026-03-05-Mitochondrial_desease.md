...
ミトコンドリア障害は大きく、<strong>原発性ミトコンドリア病</strong>と<strong>二次性ミトコンドリア障害</strong>に分けて考えると整理しやすいです。[^1]
...
本記事では、主として<strong>原発性ミトコンドリア病</strong>を扱います。
...
- <strong>組織差（tissue heterogeneity）</strong>  
  同じ個体でも、血液では変異率が低く、骨格筋や脳などで高いことがあります。したがって、血液検査だけでは病的mtDNA変異を十分に評価できない場合があります。[^1][^4][^6]

- <strong>細胞間ばらつき（cell-to-cell variance）</strong>  
  同一組織内でも細胞ごとに変異率が異なりうるため、組織全体の平均値だけでは病態を完全には表せません。[^4][^5]

- <strong>時間変化</strong>  
  ヘテロプラスミーは固定的ではなく、細胞増殖、組織更新、選択圧などにより経時的に変化しえます。特に血液系では年齢や細胞動態に伴う変動が問題になります。[^4][^5]
...
「ミトコンドリア病のミトコンドリアがどのような形をしているか」という問いに対しては、病理学的にはまず<strong>筋線維の辺縁に異常ミトコンドリアが集積して見える</strong>、というのが最も典型的です。[^19][^20]  
電子顕微鏡では、<strong>巨大化したミトコンドリア</strong>、<strong>形の不整</strong>、<strong>クリステ構造の異常</strong>、時に<strong>paracrystalline inclusions</strong>などが観察されることがありますが、これらは必ずしも特異的ではありません。[^18][^20]
...
```mermaid
flowchart TD
  A[1962: Luft et al. が、ミトコンドリア呼吸調節異常を伴う高代謝患者を報告し、ミトコンドリア病概念の起点となる] --> B[1988: 病的mtDNA変化がヒト疾患と結び付けられ、分子遺伝学時代が始まる]
  B --> C[2015-09-08: Idebenone（Raxone）が、LHONに対してEUで exceptional circumstances の下で承認]
  C --> D[2023: Elamipretide Phase 3（MMPOWER-3）が、PMMで主要評価項目に有意差なしと報告]
  D --> E[2025: Sonlicromanol（KH176）の m.3243A>G PMD に対する Phase 2b プログラム論文が公表]
  E --> F[2025: TK2 deficiency に対する pyrimidine nucleos(t)ide therapy が、死亡リスク低下を報告（Neurology）]
  F --> G[2025: Lenadogene nolparvovec（GS010）が、ND4-LHONに対する5年アウトカムを報告（RESTORE追跡）]
```
